...
function teknorova_getConfigArray($params) {
    ...
    $customfields = teknorova_parse_cache('config_settings', 512, function () {
        ...
        if(strlen($params['API_Password'])<1) {
            $sysMsg = 'Please enter your username and password';
        }else{

            require_once __DIR__ . '/lib/Teknorova.php';
            ...
            $sysMsg = teknorova_parse_cache('user_6'.md5($password), 100, function () use ($password) {
               //Burayı düzelt
                ...
                if ($details['result'] != 'OK') {
                    ...
                    $balances[]= "<b>".number_format($details['balance_try'],2)."₺</b>";
                    $balances[]= "<b>".number_format($details['balance_usd'],2)."$</b>";
...
function teknorova_GetNameservers($params) {
    require_once __DIR__.'/lib/Teknorova.php';
    ...
    // Log request
    logModuleCall("teknorova",
        substr(__FUNCTION__, 10),
        $dna->getRequest(),
        $dna->getResponse(),
        $values,
        [$password]
...
function teknorova_SaveNameservers($params) {
...
function teknorova_GetRegistrarLock($params) {
...
function teknorova_SaveRegistrarLock($params) {
...
function teknorova_RegisterDomain($params) {
    ...
    if($dna->isTrTLD($params["sld"] . "." . $params["tld"])){
        $addionalfields=teknorova_parse_trcontact($params);
    }
    ...
    $result = $dna->RegisterWithContactInfo(
        ...
        [
            ...
            "Administrative" => teknorova_parse_clientinfo($params),
            // Billing contact
            "Billing" => teknorova_parse_clientinfo($params),
            // Technical contact
            "Technical" => teknorova_parse_clientinfo($params),
            // Registrant contact
            "Registrant" => teknorova_parse_clientinfo($params),
        ],
        // Nameservers
        $nameServers,
...
function teknorova_TransferDomain($params) {
...
function teknorova_RenewDomain($params) {
    ...
    $dna = new Teknorova($username,$password,$testmode);
...
function teknorova_GetContactDetails($params) {
    ...
    {
        $values = [
            'RegistrantContact'=>teknorova_parse_contact($result["data"]["contacts"]["Registrant"]),
            'AdministrativeContact'=>teknorova_parse_contact($result["data"]["contacts"]["Administrative"]),
            'BillingContact'=>teknorova_parse_contact($result["data"]["contacts"]["Billing"]),
            'TechnicalContact'=>teknorova_parse_contact($result["data"]["contacts"]["Technical"]),
        ];
...
function teknorova_SaveContactDetails($params) {
    ...
    // DOMAIN NAME
        $params["sld"].".".$params["tld"],
        [
            "Administrative" => teknorova_parse_clientinfo($params["contactdetails"]["AdministrativeContact"]),
            "Billing" =>teknorova_parse_clientinfo($params["contactdetails"]["BillingContact"]),
            "Technical" =>teknorova_parse_clientinfo($params["contactdetails"]["TechnicalContact"]),
            "Registrant" => teknorova_parse_clientinfo($params["contactdetails"]["RegistrantContact"])
...
function teknorova_GetEPPCode($params) {
...
function teknorova_RegisterNameserver($params) {
    require_once __DIR__.'/lib/Teknorova.php';
...
function teknorova_ModifyNameserver($params) {
...
function teknorova_DeleteNameserver($params) {
    require_once __DIR__.'/lib/Teknorova.php';
...
function teknorova_IDProtectToggle($params) {
    require_once __DIR__.'/lib/Teknorova.php';
...
function teknorova_GetDNS($params)
{
...
function teknorova_SaveDNS($params)
{
    $values["error"] = "DNS Management does not supported by Domain Name API!!!";
...
function teknorova_CheckAvailability($params) {
    require_once __DIR__.'/lib/Teknorova.php';
    ...
    $exchange_rates = teknorova_exchangerates();
...
function teknorova_GetDomainSuggestions($params) {
    require_once __DIR__.'/lib/Teknorova.php';
...
function teknorova_GetTldPricing($params) {
    ...
    $convertable_currencies = teknorova_exchangerates();
...
function teknorova_Sync($params) {
    ...
    // Log request
    logModuleCall("teknorova",
        "GetDetails_FROM_".substr(__FUNCTION__, 10),
        $dna->getRequest(),
        $dna->getResponse(),
        $values,
        [$password]
...
function teknorova_TransferSync($params) {
...
function teknorova_AdminDomainsTabFields($params){


    $regs = Illuminate\Database\Capsule\Manager::table('tblregistrars')->where('registrar', 'teknorova')->get();
...
function teknorova_parse_contact($params) {
...
function teknorova_parse_clientinfo($params) {
...
function teknorova_parse_trcontact($contactDetails) {
    $cf = [];
...
function teknorova_parse_cache($key,$ttl,$callback){

    //Long usernames can cause issues with the tblconfiguration table setting column
    $cache_key = "teknorova_".md5($key); //Exact 46 character
        ...
        try {
            ...
            Capsule::table('tblconfiguration')
                   ->insert([
                       'setting' => $cache_key,
                       'value'      => '',
                       'updated_at' => $currentTime // Başlangıç `updated_at` değeri
                   ]);
        } catch (\Exception $e) {
    ...
    if (strtotime($token_row->updated_at) < (time() - 600)) {
        ...
        Capsule::table('tblconfiguration')
               ...
               ->update([
                   ...
                   'updated_at' => date('Y-m-d H:i:s') // Güncel zamanı ayarla
               ]);
...
function teknorova_exchangerates() {
    $rates = [];

    $rates = teknorova_parse_cache('currency_data', 1800, function () {
